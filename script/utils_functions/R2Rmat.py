...
def R2Rmat(R, n_comps=1, degree = False):
    #function use to make the radian angle vector [1x3] into matrix [3x3] for the projection function of the renderer
    # ONLY TENSOR VARIABLE ARE ALLOWED
    ...
    if(degree): #angle given in degree
        alpha = m.radians(R[0]) #already in radian
        beta = m.radians(R[1])
        gamma =  m.radians(R[2])
